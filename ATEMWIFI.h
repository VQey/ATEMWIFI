...
class ATEMWIFI
{
  private:
	...
	unsigned long _isConnectingTime;	// Set to millis() after the connect() function was called - and it will force runLoop() to finish the connection session.
		...
		// Selected ATEMWIFI State values. Naming attempts to match the switchers own protocol names
		// Set through _parsePacket() when the switcher sends state information
		// Accessed through getter methods
	uint8_t _ATEMWIFI_VidM;		// Video format used: 525i59.94 NTSC (0), 625i50 PAL (1), 720p50 (2), 720p59.94 (3), 1080i50 (4), 1080i59.94 (5)
	uint16_t _ATEMWIFI_PrgI;		// Program input
	uint16_t _ATEMWIFI_PrvI;		// Preview input
	uint8_t _ATEMWIFI_TlIn[16];	// Inputs 1-16, bit 0 = Prg tally, bit 1 = Prv tally. Both can be set simultaneously.
	boolean _ATEMWIFI_TrPr;		// Transition Preview: Is it on or not?
	uint8_t _ATEMWIFI_TrSS_KeyersOnNextTransition;	// Bit 0: Background; Bit 1-4: Key 1-4
	uint8_t _ATEMWIFI_TrSS_TransitionStyle;			// 0=MIX, 1=DIP, 2=WIPE, 3=DVE, 4=STING
	boolean _ATEMWIFI_KeOn[4];	// Upstream Keyer 1-4 On state
	boolean _ATEMWIFI_DskOn[2];	// Downstream Keyer 1-2 On state
	boolean _ATEMWIFI_DskTie[2];	// Downstream Keyer Tie 1-2 On state
	uint8_t _ATEMWIFI_TrPs_frameCount;	// Count down of frames in case of a transition (manual or auto)
	uint16_t _ATEMWIFI_TrPs_position;	// Position from 0-1000 of the current transition in progress
	boolean _ATEMWIFI_FtbS_state;       // State of Fade To Black, 0 = off and 1 = activated
	uint8_t _ATEMWIFI_FtbS_frameCount;	// Count down of frames in case of fade-to-black
	uint8_t	_ATEMWIFI_FtbP_time;		// Transition time for Fade-to-black
	uint8_t	_ATEMWIFI_TMxP_time;		// Transition time for Mix Transitions
	uint16_t _ATEMWIFI_AuxS[3];	// Aux Outputs 1-3 source
	uint8_t _ATEMWIFI_MPType[2];	// Media Player 1/2: Type (1=Clip, 2=Still)
	uint8_t _ATEMWIFI_MPStill[2];	// Still number (if MPType==2)
	uint8_t _ATEMWIFI_MPClip[2];	// Clip number (if MPType==1)
	uint16_t _ATEMWIFI_AMLv[2];	// Audio Meter Levels for a specific channel, see ATEM_AMLv_channel
	uint8_t _ATEMWIFI_AMLv_channel;		// The channel to read audio levels for.
	uint8_t _ATEMWIFI_AudioChannelMode[13];	// Audio channel mode (ON=1, AFV=2, OFF=0/other)
	
	
  public:
	char _ATEMWIFI_pin[17];		// String holding the id of the mixer, for instance "ATEM 1 M/E Produ"
	uint8_t	_ATEMWIFI_ver_m;	// Firmware version, "left of decimal point" (what is that called anyway?)
	uint8_t	_ATEMWIFI_ver_l;	// Firmware version, decimals ("right of decimal point")

    ATEMWIFI();
    ATEMWIFI(const IPAddress ip, const uint16_t localPort);
 ...
 ********************************/
	...
	bool hasInitialized();
	uint16_t getATEMWIFI_lastRemotePacketId();
	uint8_t getATEMWIFImodel();
...
/********************************
 * ATEMWIFI Switcher Change methods
